...
// --- Integração opcional com Firebase Admin e fetch ---
// Nota: para funcionar no Render, defina as variáveis:
// - FIREBASE_SERVICE_ACCOUNT_JSON (conteúdo JSON do service account)
// - FIREBASE_DATABASE_URL (ex: https://<seu-projeto>.firebaseio.com)
// ou use GOOGLE_APPLICATION_CREDENTIALS apontando para um arquivo secreto.
let admin = null;
let firebaseInitialized = false;
let fetchImpl = typeof fetch !== 'undefined' ? fetch : null;
try { if (!fetchImpl) { fetchImpl = require('node-fetch'); } } catch (e) {}
...
function initFirebaseIfPossible() {
  ...
  try {
    ...
    if (saJson && dbURL) {
      ...
      admin.initializeApp({ credential: admin.credential.cert(creds), databaseURL: dbURL });
      firebaseInitialized = true;
      console.log('[cors-anywhere] Firebase Admin inicializado via FIREBASE_SERVICE_ACCOUNT_JSON');
      return;
    }
    // Alternativa: GOOGLE_APPLICATION_CREDENTIALS + FIREBASE_DATABASE_URL
    if (process.env.GOOGLE_APPLICATION_CREDENTIALS && dbURL) {
      admin.initializeApp({ credential: admin.credential.applicationDefault(), databaseURL: dbURL });
      firebaseInitialized = true;
      console.log('[cors-anywhere] Firebase Admin inicializado via GOOGLE_APPLICATION_CREDENTIALS');
    }
  } catch (err) {
    console.error('[cors-anywhere] Falha ao inicializar Firebase Admin:', err);
  }
}
...
// --- Endpoint GET /cresesb/update ---
// Uso: /cresesb/update?lat=-22.9&lng=-43.2&company=MinhaEmpresa
async function cresesbUpdateEndpoint(req, res) {
  try {
    if (req.method !== 'GET') return false;
    const reqUrl = new URL(req.url, `http://${req.headers.host}`);
    if (reqUrl.pathname !== '/cresesb/update') return false;

    // Validação de parâmetros
    const latStr = reqUrl.searchParams.get('lat');
    const lngStr = reqUrl.searchParams.get('lng');
    const company = (reqUrl.searchParams.get('company') || 'default').trim();
    if (!latStr || !lngStr) {
      res.writeHead(400, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ error: 'Parâmetros obrigatórios ausentes: lat e lng' }));
      return true;
    }
    ...
    if (!fetchImpl) {
      ...
      res.end(JSON.stringify({ error: 'fetch indisponível. Use Node 18+ ou instale node-fetch.' }));
      return true;
    }

    // Monta o payload igual ao usado no front-end
    const lngAbsStr = String(Math.abs(longitude));
    const formBody = new URLSearchParams();
    formBody.append('latitude_dec', String(latitude));
    formBody.append('latitude', String(latitude));
    formBody.append('hemi_lat', latitude >= 0 ? '1' : '0');
    formBody.append('longitude_dec', lngAbsStr);
    formBody.append('longitude', String(longitude));
    formBody.append('formato', '1');
    formBody.append('lang', 'pt');
    formBody.append('section', 'sundata');

    // Faz a requisição direta ao CRESESB (sem proxy, o servidor não precisa de CORS)
    const cresesbResp = await fetchImpl('https://cresesb.cepel.br/index.php', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Origin': 'https://cresesb.cepel.br',
        'Referer': 'https://cresesb.cepel.br/index.php',
      },
      body: formBody.toString(),
    });
    if (!cresesbResp.ok) {
      res.writeHead(502, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ error: `CRESESB respondeu com status ${cresesbResp.status}` }));
      return true;
    }
    const htmlText = await cresesbResp.text();

    // Parse simples do HTML para extrair linha do plano Horizontal
    const parseResult = parseCresesbHtml(htmlText);
    const payload = {
      fonte: 'CRESESB/CEPEL',
      params: {
        latitude,
        longitude,
        hemi_lat: latitude >= 0 ? 1 : 0,
        longitude_dec: Number(lngAbsStr)
      },
    ...
    // Salvar no Realtime Database (Admin SDK se disponível, senão REST)
    initFirebaseIfPossible();
    const coordKey = `${String(latitude).replace(/\./g, '_')}_${String(longitude).replace(/\./g, '_')}`;
    const path = `cresesb/${company}/${coordKey}`;

    if (admin && firebaseInitialized) {
      await admin.database().ref(path).set(payload);
      res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ status: 'ok', saved: true, mode: 'admin', path, monthly: payload.monthly, avg: payload.avg, updatedAt: payload.updatedAt }));
      return true;
    } else {
      // Fallback REST (precisa que o nó cresesb permita escrita nas regras)
      const dbURL = process.env.FIREBASE_DATABASE_URL
        || process.env.DATABASE_URL
        || 'https://projetogdautomatizado-default-rtdb.firebaseio.com';

      let saved = false;
      try {
        const restResp = await fetchImpl(`${dbURL}/${path}.json`, {
          method: 'PUT',
    ...
    console.error('[cors-anywhere] Erro no endpoint /cresesb/update:', err);
    try {
      res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ error: err && err.message ? err.message : 'Erro interno' }));
    } catch (_) {}
    return true;
  }
}

// Parser simplificado para o HTML do CRESESB
function parseCresesbHtml(html) {
  try {
    // Extrai todas as tabelas com class tb_sundata
    const tables = [];
    html.replace(/<table[^>]*class="[^"]*tb_sundata[^"]*"[^>]*>[\s\S]*?<\/table>/gi, (m) => { tables.push(m); return m; });
    let horizontalMonthly = null;

    for (const tableHtml of tables) {
      // Procura por linhas do tbody
      const rows = [];
      tableHtml.replace(/<tbody[^>]*>[\s\S]*?<\/tbody>/i, (tbody) => {
        tbody.replace(/<tr[^>]*>[\s\S]*?<\/tr>/gi, (row) => { rows.push(row); return row; });
        return tbody;
      });

      for (const rowHtml of rows) {
        const cells = [];
        rowHtml.replace(/<td[^>]*>([\s\S]*?)<\/td>/gi, (m, inner) => {
          // Remove tags internas e espaços
          const text = String(inner).replace(/<[^>]*>/g, '').replace(/&nbsp;/g, ' ').trim();
          cells.push(text);
          return m;
        });
        if (cells.length < 17) continue;
        const plano = (cells[1] || '').toLowerCase();
        if (plano.includes('horizontal')) {
          const mesVals = [];
          for (let i = 3; i <= 14; i++) {
            const v = parseFloat(String(cells[i]).replace(',', '.'));
            mesVals.push(Number.isFinite(v) ? v : NaN);
          }
          horizontalMonthly = mesVals.map(v => (Number.isFinite(v) ? v : 0));
          break;
        }
      }
      if (horizontalMonthly) break;
    }

    let avg = 4.8;
    if (horizontalMonthly && horizontalMonthly.length === 12) {
      const sum = horizontalMonthly.reduce((acc, v) => acc + (Number.isFinite(v) ? v : 0), 0);
      avg = sum / 12;
    }
    return { monthly: horizontalMonthly || Array(12).fill(4.8), avg };
  } catch (e) {
    console.warn('[cors-anywhere] Falha ao parsear HTML do CRESESB, usando defaults:', e);
    return { monthly: Array(12).fill(4.8), avg: 4.8 };
  }
}
...
if (options.httpsOptions) {
  server = require('https').createServer(options.httpsOptions, (req, res) => {
    // Endpoints personalizados
    if (cresesbUpdateEndpoint(req, res)) return;
    if (statusEndpoint(req, res)) return;
