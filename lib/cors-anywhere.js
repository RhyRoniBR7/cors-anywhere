...
async function cresesbUpdateEndpoint(req, res) {
  try {
    ...
    if (!admin || !firebaseInitialized) {
      res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ error: 'Firebase Admin não configurado. Defina FIREBASE_SERVICE_ACCOUNT_JSON e FIREBASE_DATABASE_URL.' }));
      return true;
    }
    ...
    res.end(JSON.stringify({ status: 'ok', saved: true, path, monthly: payload.monthly, avg: payload.avg, updatedAt: payload.updatedAt }));
    return true;
  } catch (err) {
